---
name: grove
description: Start, stop, check, or read logs of local dev servers (frontend, backend, worker) for the current worktree via the Grove menu bar app. Use whenever you need a dev server running (e.g. to test a UI change in the browser), a server seems down, or you need a localhost URL.
---
...
- `grove up fe --takeover`: needed when another worktree already holds the port (e.g. Troupe's be/fe use fixed ports 3000/3001). Only use it when the user wants *this* worktree served, because it stops the other one.
